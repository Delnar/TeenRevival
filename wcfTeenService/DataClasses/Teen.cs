...
namespace wcfTeenService.DataClasses
{
    ...
    public class Teen
    {
        ...
        static public int GetNextKey()
        {
            var NextIdx = 0;
            using (var tran = Settings.Engine.GetTransaction())
            {
                var MaxRecord = tran.Max<int, DbMJSON<Teen>>("Teens");
                NextIdx = MaxRecord?.Key ?? 0;  // If no records it defaults to 0...
                NextIdx++; // Get the next seed. :)
            }
            return NextIdx;
        }
        ...
        static public List<Teen> GetAllTeens()
        {
            ...
            using (var trans = Settings.Engine.GetTransaction())
            {
                ret = trans.SelectForward<int, DbMJSON<Teen>>("Teens").Select(x => x.Value.Get).ToList();
            }
            return ret;
        }

        static public List<Teen> GetAllTeens_Entity()
        {
            List<Teen> ret = new List<Teen>();
        ...
        private List<DateTime> _lstAttendanceDates = new List<DateTime>();
        ...
        public List<DateTime> lstAttendanceDates
        {
            get { return _lstAttendanceDates; }
            set { _lstAttendanceDates = value; }
        }
        ...
        public void ReadFromDB()
        {
            using (var tran = Settings.Engine.GetTransaction())
            {
                foreach(var row in tran.SelectForwardFromTo<int, DbMJSON<Teen>>("Teens", id, true, id, true))
                {
                    var n = row.Value.Get;
                    this.CopyFrom(n);
                }                
        ...
        {
            using (var tran = Settings.Engine.GetTransaction(DBreeze.eTransactionTablesLockTypes.EXCLUSIVE, "Teens"))
            {
                tran.Insert<int, DbMJSON<Teen>>("Teens", this.id, this);
                tran.Commit();
            }
        }

        private void CopyFrom(Teen src)
        {
            id = src.id;
            FirstName = src.FirstName;
            LastName = src.LastName;
            PhoneArea = src.PhoneArea;
            PhonePrefix = src.PhonePrefix;
            PhonePostfix = src.PhonePostfix;
            Street1 = src.Street1;
            Street2 = src.Street2;
            City = src.City;
            State = src.State;
            ...
            Grade = src.Grade;
            GuestOf = src.GuestOf;
            GuestOfKey = src.GuestOfKey;

            LstAttended = new List<bool>();
            LstDecision = new List<string>();
            lstAttendanceDates = new List<DateTime>();
                Enumerable.Range(0, src.LstAttended?.Count() ?? 0).ToList()
                          .ForEach(rec => {
                            LstAttended.Add(src.LstAttended[rec]);
                            });
            Enumerable.Range(0, src.LstDecision?.Count() ?? 0).ToList()
                      .ForEach(rec => {
                          LstDecision.Add(src.LstDecision[rec]);
                      });
            Enumerable.Range(0, src.lstAttendanceDates?.Count() ?? 0).ToList()
                      .ForEach(rec => {
                          lstAttendanceDates.Add(src.lstAttendanceDates[rec]);
                      });
        }
